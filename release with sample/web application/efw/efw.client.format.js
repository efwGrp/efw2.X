/**
 * efw framework client format library
 * @author Chang Kejun
 * the value of rounder:  UP,DOWN,CEILING,FLOOR,HALF_UP,HALF_DOWN,HALF_EVEN
 */
///////////////////////////////////////////////////////////////////////////////
var EfwClientFormat=function(){};
///////////////////////////////////////////////////////////////////////////////
EfwClientFormat.prototype={
	"formatNumber":function(value,formatter,rounder){
		...
		if (isPercentage) num*=100;// if percentage, number will be multiplied by 100.
		if(isNaN(num)) return "";//if it is not number return ""
	    //round the number string
	    var numberStr=""+EfwClientFormat.prototype._roundNumber(num,ft.maxFrac,rounder);
		if(isNaN(numberStr)) return "";//if it is not number return ""
	
	    var negative = false;//the var will be used in the next
	    if (numberStr.charAt(0) == "-") {//if the number string begins with - ,negative is true
			negative = true;
			numberStr = numberStr.substring(1);
	    }else if (numberStr.charAt(0) == "+") 
	    	numberStr = numberStr.substring(1);//if the number string begins with +,negative is false
	    ...
	    else intStr = numberStr;//or the number string is all for integer part
	    ...
	    for (var i=fracStr.length; i<ft.minFrac; i++)//if minFrac=4 then 1.12 --> 1.1200
	        fracStr = fracStr + '0';
	    while (fracStr.length > ft.minFrac && fracStr.charAt(fracStr.length-1) == '0')//if minFrac=2 then 1.1200 --> 1.12)
	        fracStr = fracStr.substring(0,fracStr.length-1);
	    for (var i=intStr.length; i<ft.minInt; i++) //if minInt=4 then 034 --> 0034
	    	intStr = '0' + intStr;
	    while (intStr.length > ft.minInt && intStr.charAt(0) == '0')//if minInt=4 then 00034 --> 0034)
	    ...
	    var formattedValue;
	    if (fracStr.length > 0) formattedValue = ft.prefix + intStr + '.' + fracStr + ft.suffix;
	    else formattedValue = ft.prefix + intStr + ft.suffix;
...
//=============================================================================
	"parseNumber":function(value,formatter){
		...
		var ft=this._initNumberFormat(formatter);//get formatter object
		var pIndex = value.indexOf(ft.prefix);//get the prefix position it should be 0
		//if the suffix is blank, the suffix position will the the last position or get the position
		var sIndex = (ft.suffix == "")? value.length : value.indexOf(ft.suffix, ft.prefix.length+1);
		var isPercentage = ft.suffix && ft.suffix.charAt(0) === "%";//if percentage, number will be cut by 100.
		//if the prefix position and the suffix position is corrected, get the digital string
		if (pIndex == 0 && sIndex > 0) {
			...
			if (isNaN(value))return null;//if it is not number, return null;
			value = 0 + new Number(value);
			if(isPercentage){
				value=value/100;
				value=EfwClientFormat.prototype._roundNumber(value,ft.maxFrac+2);
			}
			return value;
		}else{
...
//=============================================================================
	"_roundNumber":function (num, precision, mode) {
		if (precision === null || precision === undefined) precision = 0;
		if(mode!="UP"&&mode!="DOWN"&&mode!="CEILING"&&mode!="FLOOR"&&
		   mode!="HALF_UP"&&mode!="HALF_DOWN"&&mode!="HALF_EVEN")mode = "HALF_EVEN";
		
		var multi="1";
		for(var i=0;i<precision;i++){multi+="0";}
		multi=new Number(multi);
		num*=multi;
	
		var numberStr=""+num;
		if(isNaN(numberStr))return null;
	    var negative = false;//the var will be used in the next
	    ...
	    if (numberStr.charAt(0) == "-") {//if the number string begins with - ,negative is true
			negative = true;
			numberStr = numberStr.substring(1);
	    }else if (numberStr.charAt(0) == "+") //if the number string begins with +,negative is false
	      numberStr = numberStr.substring(1);
		...
		case "HALF_EVEN":case "HALF_DOWN":case "HALF_UP":
	        ...
	        if (firstFracPartDigit > 5) {
				...
				if (mode === "HALF_UP") shouldRoundUp = true;
	        ...
	        if (shouldRoundUp) intStr = ""+(1+new Number(intStr));
	        break;
	    }
		numberStr=""+intStr/multi;
		if(negative)numberStr="-"+numberStr;
	    return  0+new Number(numberStr);
	},
//=============================================================================
	"_initNumberFormat":function(formatter){
		var ret={	prefix:"",			//the string before the first digital char in formatter
					...
					maxFrac:0	};		//the max length of fraction part .0## - 3
	    ...
	    var numberStr = formatter.replace(/[^0#,.]/g , "");//the number string without prefix and suffix
	    ...
	    fracStr = fracStr.replace(/[,]|[.]+/g , "");//remove [,] [.] in fraction part
	
	    ret.maxFrac = fracStr.length;//the length of fraction part formatter is the max length of fraction part
	    ...
	    if (tmp.length > ret.minInt)//if the length of zero string is bigger than 1
	      ret.minInt = tmp.length;//the min length of integer is the length of zero string
	    tmp = fracStr.replace(/[^0]/g , "");//the zero string in fraction part
	    ret.minFrac = tmp.length;//the min length of fraction is the length of zero string
...
//=============================================================================
	"parseDate":function(value,formatter){
		...
		if (!value) return null; //if value is blank return null
		var ft=this._initDateFormat(formatter);//get formatter object
	    var ret = {year: 1970, month: 1, day: 1, hour: 0, min: 0, sec: 0, msec: 0};
	
	    for (var i = 0; i < ft.length; i++) {
	       if (value == "") return null; // parse error!!
	       var pattern=ft[i];
	       ...
	       if (value === null) return null; // parse error!!
	    }
	    if (value != "") return null; // parse error!!
	    return new Date(ret.year,ret.month - 1,ret.day,ret.hour,ret.min,ret.sec,ret.msec);
	},
//=============================================================================
	"_formatDateToNumber":function(value,formatter){
		if(value==null) return "";//it value is not null, return ""
		if(!value.getTime) return "";//if value is not date, return ""
		var ft=this._initDateFormat(formatter);//get formatter object
...
//=============================================================================
	"_parseDateFromNumber":function(value,formatter){
		value += ""; //change value to string if it is not string
		if (!value) return null; //if value is blank return null
		var ft=this._initDateFormat(formatter);//get formatter object
	    var ret = {year: 1970, month: 1, day: 1, hour: 0, min: 0, sec: 0, msec: 0};
	    ...
	    for (var i = 0; i < ft.length; i++) {
	       var pattern=ft[i];
	       var parser=null;
	       if(pattern.charAt(0)=="y"||pattern.charAt(0)=="M"||pattern.charAt(0)=="d"
	    	||pattern.charAt(0)=="H"||pattern.charAt(0)=="m"||pattern.charAt(0)=="s"
	    	||pattern.charAt(0)=="S"){
	    ...
	    if (value != "") return null; // parse error!!
	    var retdate=new Date(ret.year,ret.month - 1,ret.day,ret.hour,ret.min,ret.sec,ret.msec);
...
// =============================================================================
	"_initDateFormat":function(formatter){
	    var ret = [];
